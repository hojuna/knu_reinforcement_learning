...
class A2CAgent:
    def __init__(self, state_size, save_dir=f"/home/comoz/main_project/knu_reinforcement_learning/project2/A2C/A2C_v6/save_model"):
    ...
    def learn(self):
        ...
        returns = advantages + torch.tensor([v.item() for v in self.values], device=self.device)
...
def worker(rank, shared_network, num_episodes,save_interval,return_dict):
...
if __name__ == "__main__":
    ...
    try:
        ...
        wandb.init(project="a2c_v1", name="a2c_parallel_gae")
        ...
        for rank in range(num_processes):
            p = mp.Process(
                target=worker,
                args=(rank, shared_network, episodes_per_process, save_interval,return_dict)
            )
            p.start()
            processes.append(p)
