...
class A2CAgent(): # GridSurvivorAgent 상속 제거
    def __init__(self, state_size, save_dir=f"/home/comoz/main_project/knu_reinforcement_learning/project2/A2C/A2C_v13/save_model"):
        super().__init__()
    ...
    def learn(self):
        ...
        # 텐서들을 GPU로 이동
        returns = []
        R = 0 if self.dones[-1] else self.values[-1].item()
    ...
    def save(self, episode):
        ...
        torch.save({
            'model_state_dict': self.network.cpu().state_dict(),
