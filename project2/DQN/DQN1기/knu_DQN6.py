...
class DQN(nn.Module):
    def __init__(self, input_channels, grid_height, grid_width, num_actions):
        ...
        # 완전 연결층 입력 크기 수정
        conv_output_size = 64 * grid_height * grid_width + 1  # +1 for hit_points
...
class DQNAgent(GridSurvivorAgent):
    def __init__(self, input_channels, grid_height, grid_width, num_actions, device, config):
        self.device = device
        self.policy_net = DQN(input_channels, grid_height, grid_width, num_actions).to(device)
        self.target_net = DQN(input_channels, grid_height, grid_width, num_actions).to(device)
...
def log_grid(grid, episode):
    ...
    plt.savefig(f"project2/savefig/grid_episode_{episode}.png")
    plt.close()
...
def main():
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if episode % checkpoint_interval == 0:
            checkpoint_path = f'project2/checkpoint/dqn_checkpoint_episode_{episode}.pth'
    ...
    # 최종 모델 저장
    torch.save(agent.policy_net.state_dict(), "dqn_grid_survivor_final.pth")
    wandb.finish()
...
if __name__ == "__main__":
    ...
    # run_trained_model("project2/checkpoint3/dqn_checkpoint_episode_16000.pth", num_episodes=10, render=True)
