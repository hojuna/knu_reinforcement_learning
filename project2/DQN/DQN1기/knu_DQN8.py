...
class AttentionDQN(nn.Module):
    def __init__(self, input_channels, grid_height, grid_width, num_actions):
        ...
        # 어텐션 레이어 정의
        self.attention_conv = nn.Conv2d(64, 1, kernel_size=1)  # 공간적 어텐션 맵 생성
        self.softmax = nn.Softmax(dim=1)  # 공간 차원에 대한 소프트맥스 적용

        # 완전 연결층
        self.fc1 = nn.Linear(64 * grid_height * grid_width + 1, 512)  # +1은 hit_points
        self.fc2 = nn.Linear(512, num_actions)

    def forward(self, grid, hit_points):
        # CNN 통과
        x = F.relu(self.conv1(grid))
        x = F.relu(self.conv2(x))  # 출력 크기: (batch_size, 64, grid_height, grid_width)

        # 어텐션 가중치 계산
        attn_weights = self.attention_conv(x)  # (batch_size, 1, grid_height, grid_width)
        attn_weights = attn_weights.view(x.size(0), -1)  # (batch_size, grid_height * grid_width)
        attn_weights = self.softmax(attn_weights)  # 소프트맥스 적용
        attn_weights = attn_weights.view(x.size(0), 1, grid.shape[2], grid.shape[3])  # 원래 차원으로 복원

        # 어텐션 가중치 적용
        x = x * attn_weights  # (batch_size, 64, grid_height, grid_width)

        # 플래튼
        x = x.view(x.size(0), -1)  # (batch_size, 64 * grid_height * grid_width)

        # hit_points 결합
        hit_points = hit_points.view(x.size(0), -1)  # (batch_size, 1)
        x = torch.cat((x, hit_points), dim=1)  # (batch_size, 64 * grid_height * grid_width + 1)
...
def preprocess_state(grid, hit_points):
    """환경 상태를 신경망 입력 형태로 전처리 (7x7 그리드 추출)"""
    # 에이전트의 위치를 추출
    agent_position, _ = extract_agent_info(grid)
    if agent_position is None:
        # 에이전트 위치를 찾지 못한 경우 전체 그리드를 사용
        ...
        # 그리드 패딩
        padded_grid = np.pad(grid, pad_width=half_patch, mode='constant', constant_values='E')

        # 패딩된 그리드에서 에이전트 주변의 패치 추출
        x_padded = x + half_patch
        y_padded = y + half_patch
        grid_patch = padded_grid[x_padded - half_patch:x_padded + half_patch + 1,
                                 y_padded - half_patch:y_padded + half_patch + 1]

        # 패치 인코딩
        grid_encoded = encode_grid(grid_patch)
...
def main():
    ...
    epsilon_decay = 400000  # ε 값이 더 빠르게 감소하도록 설정
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if episode % checkpoint_interval == 0:
            checkpoint_path = f'project2/checkpoint2/attention_dqn_checkpoint_episode_{episode}.pth'
    ...
    # 최종 모델 저장
    torch.save(agent.policy_net.state_dict(), "attention_dqn_grid_survivor_final.pth")
    wandb.finish()
...
if __name__ == "__main__":
    ...
    # run_trained_model("project2/checkpoint3/attention_dqn_checkpoint_episode_16000.pth", num_episodes=10, render=True)
