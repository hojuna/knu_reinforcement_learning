...
class DuelingDQN(nn.Module):
    def __init__(self, input_channels, grid_height, grid_width, num_actions):
        ...
        # 가치 스트림
        self.fc_value = nn.Linear(conv_output_size + 1, 512)  # +1 for hit_points, , +1 에이전트 방향
...
class DuelingDQNAgent(GridSurvivorAgent):
    ...
    def select_action(self, state):
        ...
        Args:
            state (tuple): 전처리된 상태 (grid_encoded, hit_points_normalized, agent_direction_encoded).
...
def encode_grid(grid):
    """그리드 데이터를 원-핫 인코딩하여 4채널 텐서로 변환 (B, H, K, Agent Position + Direction)"""
    channels = {
        'B': 0,   # Bee
        'H': 1,   # Hornet
        'K': 2,   # Killer Bee
        'A': 3    # Agent Position + Direction (0~1 값으로 정규화된 방향 포함)
    }
    ...
    direction_map = {'ar': 0, 'ad': 1, 'au': 2, 'al': 3}
    for direction, value in direction_map.items():
        grid_encoded[channels['A']][grid == direction] = value / 3.0  # 0, 0.33, 0.67, 1.0 사이 값
...
def main():
    ...
    epsilon_start = 1.0
    epsilon_end = 0.05
    epsilon_decay = 2000000  # 감쇠 스케줄을 늘려 더 천천히 감소
    ...
    for episode in range(1, num_episodes + 1):
        ...
        if episode % checkpoint_interval == 0:
            checkpoint_path = f'project2/checkpoint3/dueling_dqn_checkpoint_episode_{episode}.pth'
...
def run_trained_model(checkpoint_path, num_episodes=10, render=True):
    ...
    for episode in range(1, num_episodes + 1):
        ...
        while not done:
            ...
            action = agent.act(state_input,0.1)
            next_state, reward, done, _ ,_= env.step(action)
            next_grid = next_state['grid']
            next_hit_points = next_state['hit_points']
...
if __name__ == "__main__":
    ...
    run_trained_model("project2/checkpoint3/dueling_dqn_checkpoint_episode_16000.pth", num_episodes=10)
