...
# 수정: direction과 hit_points를 state와 next_state에 포함시킴
...
class DuelingDQN(nn.Module):
    def __init__(self, input_channels, grid_height, grid_width, num_actions):
        ...
        # feature_projection 수정: conv_output_size + hit_points (1) + direction (4) + agent_position (2) = 7
        self.feature_projection = nn.Linear(conv_output_size + 7, self.attention_dim)
    ...
    def forward(self, grid, hit_points, direction, agent_position):
        x = self.conv1(grid)
        x = self.conv2(x)
        ...
        agent_position = agent_position.view(x.size(0), -1)  # (batch_size, 2)
        
        # 추가 피처(hit_points, direction, agent_position) 결합
        x = torch.cat((x, hit_points, direction, agent_position), dim=1)
...
class ImprovedDQNAgent(GridSurvivorAgent):
    ...
    def select_action(self, state, evaluate=False):
        grid, hit_points, direction, agent_position = state
        ...
        if evaluate:
            with torch.no_grad():
                q_values = self.policy_net(grid_tensor, hit_points_tensor, direction_tensor, agent_position_tensor)
    ...
    def act(self, state, ucb_c):
        self.steps_done += 1
        grid, hit_points, direction, agent_position = state
        grid_tensor = torch.from_numpy(grid).float().unsqueeze(0).to(self.device)
        hit_points_tensor = torch.from_numpy(hit_points).float().unsqueeze(0).to(self.device)
        direction_tensor = torch.from_numpy(direction).float().unsqueeze(0).to(self.device)
        agent_position_tensor = torch.from_numpy(agent_position).float().unsqueeze(0).to(self.device)
    ...
    def optimize_model(self):
        ...
        # 상태 전처리
        grid_batch = torch.from_numpy(np.stack([s[0] for s in state_batch])).float().to(self.device)
        hit_points_batch = torch.from_numpy(np.stack([s[1] for s in state_batch])).float().to(self.device)
        direction_batch = torch.from_numpy(np.stack([s[2] for s in state_batch])).float().to(self.device)
        agent_position_batch = torch.from_numpy(np.stack([s[3] for s in state_batch])).float().to(self.device)
        
        next_grid_batch = torch.from_numpy(np.stack([s[0] for s in next_state_batch])).float().to(self.device)
        next_hit_points_batch = torch.from_numpy(np.stack([s[1] for s in next_state_batch])).float().to(self.device)
        next_direction_batch = torch.from_numpy(np.stack([s[2] for s in next_state_batch])).float().to(self.device)
        next_agent_position_batch = torch.from_numpy(np.stack([s[3] for s in next_state_batch])).float().to(self.device)
        ...
        weights = weights.to(self.device)  # 수정된 부분
        
        # 현재 Q 값 계산
        state_action_values = self.policy_net(
            grid_batch, hit_points_batch, direction_batch, agent_position_batch
        ).gather(1, action_batch.unsqueeze(1))
        
        # 타겟 Q 값 계산
        with torch.no_grad():
            next_actions = self.policy_net(
                next_grid_batch, next_hit_points_batch, next_direction_batch, next_agent_position_batch
            ).max(1)[1].unsqueeze(1)
...
def preprocess_state(grid, hit_points, direction, agent_position):
    ...
    return grid_encoded, hit_points_normalized, direction_encoded, agent_position_normalized
...
def evaluate_agent(agent, env, num_episodes=5):
    ...
    for _ in range(num_episodes):
        ...
        position, direction = extract_agent_info(grid)  # 위치와 방향 모두 추출
        
        # 에이전트의 위치 정규화
        if position is not None:
            agent_position_normalized = np.array([position[0] / grid.shape[0], position[1] / grid.shape[1]], dtype=np.float32)
        else:
            agent_position_normalized = np.array([0.0, 0.0], dtype=np.float32)
        
        # preprocess_state 함수에 4개의 인자 전달
        state_input = preprocess_state(grid, hit_points, direction, position)
        ...
        while not done:
            ...
            position, direction = extract_agent_info(grid)
            if position is not None:
                agent_position_normalized = np.array([position[0] / grid.shape[0], position[1] / grid.shape[1]], dtype=np.float32)
            else:
                agent_position_normalized = np.array([0.0, 0.0], dtype=np.float32)
    ...
    for episode in range(1, config["num_episodes"] + 1):
        ...
        while not done and step_count < config["max_steps_per_episode"]:
            for i, agent in enumerate(agents):
                ...
                next_position ,next_direction = extract_agent_info(next_grid)
                next_state_input = preprocess_state(next_grid, next_hit_points, next_direction,next_position)
                agent.memory.push(
