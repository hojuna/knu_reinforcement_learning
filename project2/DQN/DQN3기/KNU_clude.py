...
class ImprovedDQNAgent(GridSurvivorAgent):
    def __init__(self, input_channels, grid_height, grid_width, num_actions, device, config):
        ...
        # 데이터 타입을 float32로 명시적 지정
        self.policy_net.double().float()  # double()을 제거하고 float32로 변환
        self.target_net.double().float()  # double()을 제거하고 float32로 변환
        
        self.num_actions = num_actions
        self.action_counts = np.zeros(num_actions, dtype=np.float32)  # dtype 명시
...
def main(trial):
    ...
    wandb.init(
        ...
        config={
            ...
            "memory_capacity": trial.suggest_categorical("memory_capacity", [50000, 100000, 200000]),
            "num_channels": 4,
            "grid_height": 34,
            "grid_width": 34,
            "num_actions": 3,
    ...
    for episode in range(1, config["num_episodes"] + 1):
        ...
        if episode % config["checkpoint_interval"] == 0:
            for i, agent in enumerate(agents):
                torch.save(agent.policy_net.state_dict(), f"checkpoint_model_{i}_episode_{episode}.pth")
            wandb.finish()
