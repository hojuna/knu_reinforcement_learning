...
class GridAdventureRLAgent(GridAdventureAgent):
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count,has_key,current_direction):
                ...
                if not self.is_value_in_grid(k, grid):
                    ...
                    if (current_position[0] + direction_mapping[current_direction][0], current_position[1] + direction_mapping[current_direction][1]) in [(1,19),(1,13)]:
                        ...
                        if k not in self.reward_queue:
                            ...
                            elif k == "KG" and "KB" in self.reward_queue and "KBD" in self.reward_queue:
                                goal_reward+=self.goal_reward_value+self.goal_reward_value
                                self.reward_queue.append(k)

                            elif k == "KR" and "KG" in self.reward_queue and "KB" in self.reward_queue and "KGD" in self.reward_queue:
        ...
        if terminated and next_position == (24, 24):
            if 'G' not in self.reward_queue:
                goal_reward += 30000 * (1 -step_count/2704)
    ...
    def act(self, observation):
        ...
        if np.random.rand() < self.epsilon:
            ...
            # print(position[0], position[1], direction, has_key,door_state_index[0],door_state_index[1],door_state_index[2], action_index )
            # print(state_actions)
            # print(action_index)
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='2024.10.29/last_q-table_v4.pkl',epsilon=0.5)
    trained_agent, episode_rewards = agent.train(10000,False)
    # agent.q_table[5, 24, 2, 3, 1, 1,1, :]=75.92613055,  76.70681118, -20.99698984  ,76.75872356  ,75.92613055,76.86507869
    # agent.q_table[5, 24, 2, 3, 1, 1,1, 0]=75.92613055
    # agent.save_q_table()
