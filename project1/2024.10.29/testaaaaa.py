...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='/content/drive/MyDrive/knu/real_agent/BG_q_table_v4.pkl',epsilon=0.1):
        # 가능한 행동들 정의 (기본 행동)
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count,key_flag,key_flag_next):
        ...
        if self.knu_agent_actions[action_index] == self.ACTION_DROP:
            ...
            if key_flag==2 and key_flag_next==0 :
                if "DGO" in self.reward_queue and not "KGD" in self.reward_queue:
                    goal_reward += 500
...
if __name__ == '__main__':
    ...
    # agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='2024.10.29/BG_q_table_v4.pkl',epsilon=0.5)
