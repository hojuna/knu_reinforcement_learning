            ...
            for j in range(26):
                if grid[i][j] == 'W' or grid[i][j] == 'L':
                    ...
                    if distance != float('inf') and distance != 0:
                        self.bfs_table[i, j] = 1 / distance  # 스케일링 
                    else:
                        self.bfs_table[i, j] = 0
...
if __name__ == '__main__':
    ...
    # trained_agent, episode_rewards = agent.train(10000,False)

    # 학습된 에이전트를 로드하여 평가 실행
    evaluate(result_agent)

#epsilon 0.5로 시작해서 Episode: 3500 정도 실시
#epsilon 0.3으로 500~1000번정도 해주다
#epsilon 0.1로 최적경로 나올때까지 해줬습니다.