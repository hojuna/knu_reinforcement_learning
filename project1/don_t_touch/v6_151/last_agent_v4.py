...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='2024.10.29/last_q-table_v5.pkl',epsilon=0.1):
        # 가능한 행동들 정의 (기본 행동)
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count,has_key,current_direction):
        ...
        if self.knu_agent_actions[action_index] == self.ACTION_PICKUP:
            for  k in ["KB", "KG", "KR"]:
                if not self.is_value_in_grid(k, grid):
                    # 열쇠 목표 달성 확인
                    if (current_position[0] + direction_mapping[current_direction][0], current_position[1] + direction_mapping[current_direction][1]) in [(1,19),(1,13)]:
                        pickup_penalty-=10
        ...
        #         if current_position == (1,19) and current_direction == 1:
        #             if has_key == 2:
        ...
        #         elif current_position == (1,13) and current_direction == 1:
        #             if has_key == 1:
        ...
        total_reward = temp_reward + time_penalty + terminal_penalty + wall_penalty + drop_penalty +pickup_penalty

        # if goal_reward > 0 and 'G' in self.reward_queue:
        #     print(f"목표 보상: {goal_reward}, goals_step:{step_count}")
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='last_q-table_v5.pkl',epsilon=0)
    # agent.train(100)
    # agent2 = GridAdventureRLAgent(load_q_table=True, q_table_filename='last_q-table_v5.pkl',epsilon=0)
    # result_agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='last_result_q_table.pkl',epsilon=0)
    # result_agent,_,_=result_agent.train(10)
    # cnt= 153
    # while True:
    #     agent.train(1,False)
    ...
    #         print(f'최적화 중 현재 {t}')
    #         cnt=t
    #         with open('last_result_q_table.pkl', 'wb') as f:
    #             pickle.dump(agent2.q_table, f)


    # trained_agent, episode_rewards, _= agent.train(10000,False)

    # 학습된 에이전트를 로드하여 평가 실행
    # evaluate(result_agent)
    evaluate(agent)

#Episode: 3438, Total Reward: 30971.90, Epsilon: 0.3625, Goals Achieved: deque(['KB', 'DBO', 'DBL', 'KG', 'DGO', 'DGL', 'KR', 'DRO', 'DRL', 'G']), Steps: 921, Current Position: (23, 24)