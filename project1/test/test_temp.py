...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='test/test_colab_v2.pkl',epsilon=0.1):
        # 가능한 행동들 정의 (기본 행동)
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated):
        ...
        if terminated and grid[next_position[0]][next_position[1]] == 'G':
            goal_reward += 1000
    ...
    def generate_and_print_combined_table(self, grid, current_position, goal):
        """
        ...
        # 2. 최단 경로 찾기
        path = self.bfs_shortest_path(grid, current_position, goal)
        if not path:
        ...
        # 4. 테이블 합산
        combined_table = self.bfs_table + binary_path
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='test/test_colab_v2.pkl',epsilon=0.1)
    # trained_agent, episode_rewards = agent.train(5)
