...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='/content/drive/MyDrive/knu/10_26_agent/q_table/knu_q_table_26_v3.pkl',epsilon=0.1):
        # 가능한 행동들 정의 (기본 행동)
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count):
        ...
        #             if item not in self.reward_queue:
        #                 bl=False
        #                 temp_item = item
        #                 break
        #         if bl:
        #             self.item = False
        #             goal_reward += 100
        #             self.reward_queue.append(temp_item)
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='2024.10.26/knu_q_table_26_v4.pkl',epsilon=0)
    trained_agent, episode_rewards = agent.train(0)
