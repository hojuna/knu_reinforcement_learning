...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='knu_q_table_improved.pkl'):
        # 가능한 행동들 정의 (기본 행동)
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated):
        ...
        #이동 보상 먹었으면 초기화
        self.bfs_table[next_position[0], next_position[1]] = 0
        ...
        # 목표 달성 확인
        if current_goal:
            if current_goal.startswith('K'):  # 열쇠 목표인 경우

                if self.ACTION_SPACE[action_index] == self.ACTION_PICKUP:
                    ...
                    for k in key:
                        if not self.is_value_in_grid(k,grid):
                            if k == current_goal:
                                ...
                                self.current_goal_index += 1  # 다음 목표로 이동
                                break
            elif current_goal.startswith('D'):  # 문 목표인 경우
                # 에이전트가 현재 위치에서 문을 열었는지 확인
                 if self.ACTION_SPACE[action_index] == self.ACTION_UNLOCK:
                    print(f"문 열림: 시도는 하는거지?")
                    door_open_state = current_goal  # 문이 열린 상태
                    for i in range(self.state_size[0]):
                        for j in range(self.state_size[1]):
                            if grid[i][j] == door_open_state:
                                goal_reward = self.goal_reward_value
                                self.current_goal_index += 1  # 다음 목표로 이동
                                break
        ...
        if goal_reward > 0:
            print(f"목표 보상: {goal_reward}, goals:{self.goals[self.current_goal_index]}")
    ...
    def update_bfs_table(self, grid):
        ...
        goal_position = self.find_goal_position(goal)
        for i in range(26):
            for j in range(26):
                if grid[i][j] == 'W' or grid[i][j] == 'L':
                    ...
                    if distance != float('inf') and distance != 0:
                        self.bfs_table[i, j] = 10 / distance
                    else:
                        self.bfs_table[i, j] = 0
    ...
    def train(self, num_episodes=10000):
        ...
        while True:
            ...
            while not done:
                ...
                # 환경에서 실제로 행동 수행
                observation_next, env_reward, terminated, truncated, _ = env.step(action)
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=False, q_table_filename='knu_q_table_improved.pkl')
