...
class GridAdventureRLAgent(GridAdventureAgent):
    ...
    def find_faced_cell(self, current_position, direction,grid):
        """방향과 현재 위치, 맵을 받아 현재 마주하고 있는 셀을 반환"""

        next_position = (current_position[0] + self.direction_grid_mapping[direction][0], current_position[1] + self.direction_grid_mapping[direction][1])

        return grid[next_position[0]][next_position[1]]


    def is_value_in_grid(self, value, grid):
        """맵에 해당 값이 존재하는지 찾는 함수"""
        for row in grid:
            if value in row:
                return True
        return False
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count,has_key,current_direction):
        ...
        if self.knu_agent_actions[action_index] == self.ACTION_PICKUP:
            for  k in ["KB", "KG", "KR"]:
                if not self.is_value_in_grid(k, grid):
                    # 열쇠 목표 달성 확인
                    if (current_position[0] + self.direction_grid_mapping[current_direction][0], current_position[1] + self.direction_grid_mapping[current_direction][1]) in  self.drop_point:
                        pickup_penalty-=10
        ...
        if (
            terminated and 
            next_position == (24, 24) and
            not 'G' in self.reward_queue
        ):
                goal_reward += 25000 * (1 -step_count/2704)
        ...
        total_reward = temp_reward + time_penalty + terminal_penalty + wall_penalty  +pickup_penalty
    ...
    def train(self, num_episodes=10000, show=False):
        ...
        while episode < num_episodes:
            ...
            while not done:
                ...
                custom_reward , cnt_pos = self.custom_reward(
                    current_position, action_index, state, grid, done, cnt_pos ,step_count,has_key,current_direction
        ...
        return self, episode_rewards, step_count
...
if __name__ == '__main__':
    ...
    # agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='knu_q-table.pkl',epsilon=0.5)
    # agent2 = GridAdventureRLAgent(load_q_table=True, q_table_filename='knu_q-table.pkl',epsilon=0.5)

    result_agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='knu_result_q-table.pkl',epsilon=0)
    evaluate(result_agent)


    # trained_agent, episode_rewards, _= agent.train(4000,False)
    # agent.epsilon=0.3
    # trained_agent, episode_rewards, _= agent.train(2000,False)
    ...
    # agent.alpha=0.05
    # agent.epsilon=0.05
    # trained_agent, episode_rewards, _= agent.train(1000,False)
    

    #여기서 최소한 step이 152~153까지는 나와줄때까지 돌렸던거 같습니다.
    # agent.epsilon=0.01
    # trained_agent, episode_rewards, step= agent.train(1000,False)



    # #코드 구조가 불안정해서 그런지 마냥 계속 돌리걸론 최적화가 잘 안되더라고요
    #여기서  더 낮은 step의 q table이 나오면 따로 저장하도록 했습니다.
    #하지만 여기서도 최적화가 모자라면 다시 0.01 앱실론으로 train 시켜줬습니다.
    #기존에는 에피소드 100번마다 저장해줬는데 여기서는 매번 q_tabbl을 저장하도록 코드를 수정해줘야 합니다.
    # cnt= step
    # while True:
    #     agent.train(1,False)
    #     _ ,_,t=agent2.train(1)
    #     if cnt>t:
    #         print(f'최적화 중 현재 {t}')
    #         cnt=t
    #         with open('knu_result_q_table.pkl', 'wb') as f:
