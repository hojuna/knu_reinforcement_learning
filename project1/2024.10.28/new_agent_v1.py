...
class GridAdventureRLAgent(GridAdventureAgent):
    ...
    def act(self, observation):
        ...
        if self.option_action == None:
            ...
            self.option_action = self.act_option(action,faced_cell)

            if faced_cell =="W" and action==self.knu_agent_actions[-1]:
                self.keys=[]
            elif faced_cell in ["KG","KB","KR"] and action==self.knu_agent_actions[-2]:
                self.keys.append(faced_cell)
        else:
    ...
    def act_option(self,action, faced_cell):
        
        #1. 파랑, 초록 열쇠면 먹고 left
        if faced_cell in ["KG",'KB'] and action == self.knu_agent_actions[3]:
            return self.knu_agent_actions[1]
        
        #2. 빨강 열쇠면 foward
        elif faced_cell in ["KR"] and action == self.knu_agent_actions[3]:
            return self.knu_agent_actions[0]
        
        #3. 문을 열면 포워드
        elif faced_cell in ["DGC","DBC","DRC"] and action == self.knu_agent_actions[-2]:
            return self.knu_agent_actions[0]
        #5. 열쇠 드랍하면 left
        elif faced_cell in ['W'] and action == self.knu_agent_actions[-1] and self.keys!=[]:
            return self.knu_agent_actions[1]
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='2024.10.28/new_q_table.pkl',epsilon=0.5)
    trained_agent, episode_rewards = agent.train(num_episodes=50000)
