...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='2024.10.27/BG_q_table_v6.pkl',epsilon=0.1):
        ...
        self.knu_agent_actions = [self.ACTION_LEFT, self.ACTION_RIGHT, self.ACTION_FORWARD, self.ACTION_PICKUP, self.ACTION_UNLOCK,self.ACTION_DROP]
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count,key_flag,key_flag_next):
            ...
            for d in opened_doors:
                if self.is_value_in_grid(d, grid):
                    if d not in self.reward_queue:
                        ...
                        if d == "DBO" and "DGO" in self.reward_queue:
                            goal_reward+=self.goal_reward_value
        ...
        if "KG" in self.reward_queue:
            temp_reward*=1.1
            if "KB" in self.reward_queue:
                temp_reward*=1.1
                if "KR" in self.reward_queue:
                    temp_reward*=1.1
        ...
        return total_reward, cnt_pos ,env_done
    ...
    def act(self, observation):
        ...
        print(f"행동: {action}, 행동 인덱스: {action_index},key_flag:{self.key_flag},current_key:{self.current_key}")
        return action_index
    ...
    def generate_and_print_combined_table(self, grid, current_position):
        ...
        # 4. 테이블 합산
        combined_table = self.bfs_table *10 + binary_path + (binary_path2 *0.7)
        # 4. 테이블 합산
    ...
    def train(self, num_episodes=10000):
        ...
        while episode < num_episodes:
            ...
            while not done:
                ...
                # 커스텀 보상 계산
                custom_reward , cnt_pos ,env_done = self.custom_reward(
                ...
                if env_done:
                    print(f"제자리 반복 10회 초과로  현재 에피소드 종료")  
                if done:
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=True, q_table_filename='2024.10.27/BG_q_table_v4.pkl',epsilon=0.1)
    trained_agent, episode_rewards = agent.train(num_episodes=0)
