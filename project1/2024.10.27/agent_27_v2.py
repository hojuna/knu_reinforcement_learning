...
class GridAdventureRLAgent(GridAdventureAgent):
    def __init__(self, load_q_table=False, q_table_filename='2024.10.27/knu_q_table_27_v2.pkl',epsilon=0.1):
        # 가능한 행동들 정의 (기본 행동)
    ...
    def custom_reward(self, current_position, action_index, next_position, grid, terminated, cnt,step_count,key_flag,key_flag_next):
        ...
        if self.knu_agent_actions[action_index] == self.ACTION_DROP:    
            if key_flag==1 and key_flag_next==0:
                if "DBO" in self.reward_queue:
                    goal_reward += 500
                elif "DGO" in self.reward_queue:
                    goal_reward += 500
    ...
    def state_find_door(self, grid):
        door_positions = {}
        find_door_list = ["DBL", "DGL", "DRL"]
        for i in range(26):
            for j in range(26):
                if grid[i][j] in find_door_list:
                    door_positions[(i, j)]=grid[i][j]
                    find_door_list.remove(grid[i][j])
    ...
    def generate_and_print_combined_table(self, grid, current_position):
        ...
        # 4. 테이블 합산
        combined_table = self.bfs_table *10 + binary_path
...
if __name__ == '__main__':
    ...
    agent = GridAdventureRLAgent(load_q_table=False, q_table_filename='2024.10.27/knu_q_table_27_v2.pkl',epsilon=0.1)
